...
\[
%
logit(P({sleepyfreq}\leq i)) = \beta_{i0}+\beta_1{carblevel}+\beta_2sleephr +\beta_3female + \beta_4ager + \beta_5winter
%
\]
...
As alternative approaches, unrestrictive Generalized Logistic Regression and Partial Proportional Odds model are fit on the data. Although they perform better according to likelihood-ratio test, we turn back to Ordinal Logistic Regression due to the following reasons.

<ul>
<li>
For Generalized Logistic Regression, first it frees all variables from the proportional odds constraint, even though the assumption is only be violated by a few of them. Also, it does not require ordinality which may lead to loss of information that comes with the original data. Lastly, its estimated probabilities can go negative and results are hard to interpret.
</li>
<li>
For Partial Proportional Odds model, predictors without the assumption constraint are still hard to interpret. Also, a closer look at the coefficient estimates tell us it does not differ much from the result of the Ordinal Logistic Regression.
</li>
</ul>

So from the results we get from Ordinal Logistic Regression model above,when compared to level __suggested range__, __Below range__ is positively related to the odds of feeling sleepy while __above range__ is negatively related to the odds of feeling sleepy.
